...
- The template editor will open.\
  ...
  **Category:** Choose from Utility, Marketing, Authentication or Carousel.\
  **Channel & Language:** Choose the WhatsApp number and message language.

<VideoLayout url={'/assets/video/template/create_a_whatsapp_template.mp4'} />
...
<VideoLayout url={'/assets/video/template/template_header.mp4'} />

**2. Body**\
The core message. Personalize it using variables like `{{name}}` or `{{orderId}}`.  
✅ Example: Hi `{{name}}`, your order `{{orderId}}` is on its way!

<VideoLayout url={'/assets/video/template/template_body.mp4'} />

📌 Always provide sample values for each variable before submitting for approval.

- To know more about variables, visit [Variables in Template](/templates/template-creation-and-editing/variables-in-template)
...
<VideoLayout url={'/assets/video/template/template_footer.mp4'} />
...
<VideoLayout url={'/assets/video/template/template_button.mp4'} />
